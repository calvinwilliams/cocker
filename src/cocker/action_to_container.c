...
int DoAction_to_container( struct CockerEnvironment *env )
{
	...
	nret = CreateContainer( env , env->cmd_para.__from_image , env->cmd_para.__to_container ) ;
	...
	nret = SnprintfAndSystem( cmd , sizeof(cmd) , "cp -rf %s/rlayer/* %s/rwlayer/" , env->image_path_base , env->container_path_base ) ;
	INTER1( "*** ERROR : SnprintfAndSystem [cp -rf %s/rlayer/* %s/rwlayer/] failed[%d] , errno[%d]\n" , env->image_path_base , env->container_path_base , nret , errno )
	EIDTE( "system [%s] ok\n" , cmd )
